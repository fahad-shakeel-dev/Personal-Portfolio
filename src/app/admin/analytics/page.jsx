...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [contactsResponse, reviewsResponse, projectsResponse] = await Promise.all([
  ...
  const filterByPeriod = (data, field, period) => {
    const now = new Date();
    let startDate;
    switch (period) {
      case "7d":
    ...
    return data.filter((item) => new Date(item[field]) >= startDate);
  };

  // Aggregate data for trends
  const aggregateTrendData = (data, field, period) => {
    const filteredData = filterByPeriod(data, field, period);
    const now = new Date();
    const days = period === "7d" ? 7 : period === "30d" ? 30 : period === "90d" ? 90 : 365;
    const trendData = Array.from({ length: days }, (_, i) => {
      const date = new Date(now - (days - 1 - i) * 24 * 60 * 60 * 1000);
      const count = filteredData.filter(
        (item) => new Date(item[field]).toDateString() === date.toDateString()
      ).length;
      return { date: date.toISOString().split("T")[0], value: count };
    });
    ...
    const previousPeriodData = filterByPeriod(
      ...
      field,
      period,
      new Date(now - days * 2 * 24 * 60 * 60 * 1000),
      new Date(now - days * 24 * 60 * 60 * 1000)
    );
    const previous = previousPeriodData.length;
    const change = previous > 0 ? ((current - previous) / previous) * 100 : 0;
    return { current, previous, change: change.toFixed(1), data: trendData.slice(-7) };
  };

  // Calculate analytics data dynamically
  const contactTrends = aggregateTrendData(contacts, "createdAt", selectedPeriod);
  const reviewTrends = aggregateTrendData(reviews, "createdAt", selectedPeriod);

  const contactAnalytics = {
    totalSubmissions: contacts.length,
    responseRate: contacts.length
      ? ((contacts.filter((c) => c.responses?.length > 0).length / contacts.length) * 100).toFixed(1)
      : 0,
    avgResponseTime: contacts.length
      ? (
          contacts
            ...
            .reduce((sum, c) => {
              ...
              const timeDiff =
                new Date(firstResponse.sentDate) - new Date(c.createdAt);
              return sum + timeDiff / (1000 * 60 * 60);
            }, 0) / contacts.filter((c) => c.responses?.length > 0).length
        ).toFixed(1) + " hours"
      : "0 hours",
    conversionRate: contacts.length
      ? ((contacts.filter((c) => c.status === "approved").length / contacts.length) * 100).toFixed(1)
      : 0,
    topInquiryTypes: Object.entries(
      contacts.reduce((acc, c) => {
        const type = c.serviceInterestedIn || "Unknown";
  ...
  const reviewsAnalytics = {
    ...
    responseRate: reviews.length
      ? ((reviews.filter((r) => r.adminReply).length / reviews.length) * 100).toFixed(1)
      : 0,
    ratingDistribution: Array.from({ length: 5 }, (_, i) => {
      ...
      return {
        rating,
        count,
        percentage: reviews.length ? ((count / reviews.length) * 100).toFixed(1) : 0,
      };
    }).filter((r) => r.count > 0),
  };
  ...
  const topProjects = projects
    .map((p) => ({
      ...
      inquiries: contacts.filter((c) => c.projectId === p.id).length,
    }))
  ...
  if (isLoading) {
    return <div className="text-center p-8 text-emerald-600 animate-pulse">Loading analytics data...</div>;
  }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-sm font-medium ${getChangeColor(contactTrends.change)}`}>
              {getChangeIcon(contactTrends.change)} {Math.abs(contactTrends.change)}%
            </span>
          </div>
          <div>
            <p className="text-2xl font-bold text-emerald-800">{contactTrends.current}</p>
            <p className="text-sm text-emerald-600">Contact Submissions</p>
          </div>
          <div className="mt-4">{renderMiniChart(contactTrends.data)}</div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-emerald-600">
              {reviewsAnalytics.averageRating}/5
            </span>
          </div>
          <div>
            <p className="text-2xl font-bold text-emerald-800">{reviewsAnalytics.totalReviews}</p>
            <p className="text-sm text-emerald-600">Total Reviews</p>
          </div>
          <div className="mt-4 flex items-center gap-1">
            {Array.from({ length: 5 }, (_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${i < Math.floor(reviewsAnalytics.averageRating) ? "text-yellow-400" : "text-gray-300"}`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-emerald-600">{projects.length}</span>
          </div>
          <div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-emerald-600">{contactAnalytics.conversionRate}%</span>
          </div>
          <div>
            <p className="text-2xl font-bold text-emerald-800">{contactAnalytics.conversionRate}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-emerald-800">Trend Overview</h2>
            <select
              value={selectedMetric}
              onChange={(e) => setSelectedMetric(e.target.value)}
              className="px-3 py-1 border border-emerald-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
          ...
          <div className="h-64 flex items-end justify-between gap-2">
            {(selectedMetric === "contacts" ? contactTrends.data : reviewTrends.data).map((point, index) => {
              const maxValue = Math.max(...(selectedMetric === "contacts" ? contactTrends.data : reviewTrends.data).map((d) => d.value), 1);
              const height = (point.value / maxValue) * 100;
              return (
                <div key={index} className="flex-1 flex flex-col items-center">
                  <div
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-emerald-800">{contactAnalytics.responseRate}%</p>
              <p className="text-sm text-emerald-600">Response Rate</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-emerald-800">{contactAnalytics.avgResponseTime}</p>
              <p className="text-sm text-emerald-600">Avg Response Time</p>
            </div>
          </div>
          <div className="space-y-3">
            <h3 className="font-medium text-emerald-800">Top Inquiry Types</h3>
            {contactAnalytics.topInquiryTypes.map((type, index) => (
              <div key={index} className="flex items-center justify-between">
                <span className="text-sm text-emerald-700">{type.type}</span>
                <div className="flex items-center gap-2">
                  <div className="w-16 bg-emerald-100 rounded-full h-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-emerald-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <p className="text-3xl font-bold text-emerald-800">{reviewsAnalytics.averageRating}</p>
              <p className="text-sm text-emerald-600">Average Rating</p>
              <div className="flex justify-center mt-2">
                {Array.from({ length: 5 }, (_, i) => (
                  <svg
                    key={i}
                    className={`w-5 h-5 ${i < Math.floor(reviewsAnalytics.averageRating) ? "text-yellow-400" : "text-gray-300"}`}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
            ...
            <div className="text-center">
              <p className="text-3xl font-bold text-emerald-800">{reviewsAnalytics.responseRate}%</p>
              <p className="text-sm text-emerald-600">Response Rate</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-emerald-800">{reviewsAnalytics.totalReviews}</p>
              <p className="text-sm text-emerald-600">Total Reviews</p>
            </div>
          </div>
          <div className="mt-6">
            <h3 className="font-medium text-emerald-800 mb-4">Rating Distribution</h3>
            <div className="space-y-2">
              {reviewsAnalytics.ratingDistribution.map((rating) => (
                <div key={rating.rating} className="flex items-center gap-3">
                  <div className="flex items-center gap-1 w-16">
                    <span className="text-sm text-emerald-700">{rating.rating}</span>
