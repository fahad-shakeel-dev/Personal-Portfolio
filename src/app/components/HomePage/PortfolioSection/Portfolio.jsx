...
//                     </p> */}
//                     <p
//   className="text-gray-600 leading-relaxed text-sm sm:text-base line-clamp-3"
//   aria-describedby={`project-title-${project.id}`}
//   itemProp="description"
// >
//   {project.description}
...
export default function PortfolioSection() {
  ...
  // Updated filters to match enum categories
  const filters = ["All", "Web Development", "Mobile Development", "Web Design", "UI/UX"]

  // Updated filter mapping to match backend categories
  ...
  const filteredProjects = useMemo(() => {
    return activeFilter === "All"
      ? projects
      : projects.filter((project) => project.category === filterToCategoryMap[activeFilter])
  }, [projects, activeFilter])

  // Show only 3 projects on mobile, 6 on larger screens
  const displayedProjects = filteredProjects.slice(0, typeof window !== "undefined" && window.innerWidth < 640 ? 3 : 6)
  ...
  {displayedProjects.slice(0, 3).map((project, index) => (
    ...
    >
      ...
      >
        ...
        {/* View More Button (only show if more than 3 projects) */}
