...
export async function POST(request) {
  ...
  try {
    ...
    const existingTestimonial = await Testimonial.findOne({ userId: user._id });
    if (existingTestimonial && user.role !== 'admin') {
      return NextResponse.json(
        { message: "You have already submitted a testimonial" },
        { status: 400 }
      );
    ...
    await testimonial.save();
    return NextResponse.json(
      { message: "Testimonial submitted for approval", testimonial },
      { status: 201 }
    );
  } catch (error) {
    console.error("POST testimonial error:", error.message);
...
export async function GET(request) {
  ...
  try {
    ...
    if (all && user?.role === 'admin') {
      ...
      query = { $or: [
        { userId: user._id },
        { status: 'approved', displayOnPortfolio: true }
      ]};
    } else {
      query.status = 'approved';
      query.displayOnPortfolio = true;
...
export async function PUT(request) {
  ...
  try {
    ...
    if (user.role === 'admin') {
      ...
      testimonial.displayOnPortfolio = displayOnPortfolio !== undefined ? displayOnPortfolio : testimonial.displayOnPortfolio;
    } else {
      testimonial.name = user.fullName;
      testimonial.email = user.email;
    }
