...
//   await dbConnect();
//   try {
//     let id, title, description, category, status, technologies, liveUrl, githubUrl, imageFile, galleryFiles,deletedGalleryImages;
//     const contentType = req.headers.get('content-type') || '';
...
//       imageFile = formData.get('image');
//       galleryFiles = formData.getAll('gallery');
//       deletedGalleryImages = formData.get('deletedGalleryImages') ? JSON.parse(formData.get('deletedGalleryImages')) : [];
//     } else if (contentType.includes('application/json')) {
//       const body = await req.json();
//       ({ id, title, description, category, status, technologies, liveUrl, githubUrl, image: imageFile, gallery: galleryFiles,deletedGalleryImages } = body);
//       technologies = technologies || [];
//       galleryFiles = galleryFiles || [];
...
//       const publicIds = deletedGalleryImages
//         .filter(url => !url.includes('placeholder.svg'))
//         .map(url => url.split('/').pop().split('.')[0]);
//       if (publicIds.length > 0) {
//         await deleteImages(publicIds);
//       }
//       galleryUrls = galleryUrls.filter(url => !deletedGalleryImages.includes(url));
//     }
...
//           })
//         );
//         const newGalleryUrls = await uploadImages(buffers);
...
//         galleryUrls = [...galleryUrls, ...newGalleryUrls];
//       } else {
//         galleryUrls = [...galleryUrls, ...galleryFiles];
//       }
//     }
...
export async function GET(req) {
  ...
  try {
    ...
    const formattedProjects = projects.map(project => ({
      ...
      userHasLiked: userId ? project.likedBy?.includes(userId) : false,
      viewedBy: project.viewedBy || [],
      features: project.features || [], // Include features in response
    }));
...
export async function POST(req) {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      status = formData.get('status');
      technologies = formData.get('technologies')?.split(',').map(tech => tech.trim()).filter(Boolean) || [];
      features = formData.getAll('features').filter(feature => feature.trim()); // Extract features
      liveUrl = formData.get('liveUrl');
      githubUrl = formData.get('githubUrl');
      imageFile = formData.get('image');
      galleryFiles = formData.getAll('gallery');
    ...
    const project = new Project({
      ...
      status,
      technologies,
      features: features || [], // Include features in the Project document
      image: imageUrl,
      gallery: galleryUrls,
      liveUrl: liveUrl || '',
      githubUrl: githubUrl || '',
...
export async function PUT(req) {
  await dbConnect();
  try {
    let id, title, description, category, status, technologies, liveUrl, githubUrl, imageFile, galleryFiles, deletedGalleryImages, features;
    const contentType = req.headers.get('content-type') || '';
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const body = await req.json();
      ({ id, title, description, category, status, technologies, liveUrl, githubUrl, image: imageFile, gallery: galleryFiles, deletedGalleryImages, features } = body);
      technologies = technologies || [];
      features = features || []; // Ensure features is an array
      galleryFiles = galleryFiles || [];
      deletedGalleryImages = deletedGalleryImages || [];
    ...
    project.status = status || project.status;
    project.technologies = technologies.length ? technologies : project.technologies;
    project.features = features.length ? features : project.features; // Update features
    project.image = imageUrl;
    project.gallery = galleryUrls;
    project.liveUrl = liveUrl || project.liveUrl;
    project.githubUrl = githubUrl || project.githubUrl;
