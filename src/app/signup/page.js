...
export default function SignupPage() {
  ...
  // Show resend link after 15 seconds if signup is successful or email is unverified
  useEffect(() => {
    let timer;
    if (signupSuccess) {
      timer = setTimeout(() => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.message === 'Email already exists but not verified') {
          toast.info('A verification email has already been sent. Please check your inbox or use the resend link below after 15 seconds.');
          setSignupSuccess(true);
        ...
        {showResendLink && signupSuccess && (
          <div className="text-center">
            ...
            <a
              ...
              className="inline-flex items-center justify-center py-2 px-6 bg-transparent text-emerald-600 font-semibold border border-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition duration-150 ease-in-out"
            >
